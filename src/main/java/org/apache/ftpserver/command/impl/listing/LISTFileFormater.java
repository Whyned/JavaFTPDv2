...
public class LISTFileFormater implements FileFormater {
    ...
    public String format(FtpFile file) {
        ...
        if(file.isSymlink()){
        	String relativesymlinkpath = file.getRelativeSymlinkPath();
        	if(relativesymlinkpath != null){
        		sb.append(" -> "+relativesymlinkpath);
        	}
    ...
    private char[] getPermission(FtpFile file) {
        ...
        permission[0] = file.isSymlink() && file.getRelativeSymlinkPath() != null ? 'l' : permission[0];
